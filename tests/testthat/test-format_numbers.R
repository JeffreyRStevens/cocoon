...
test_that("format_chr() formats properly", {
  suppressMessages(expect_error(format_chr(3),
                                "Input must be a character string"))
  suppressMessages(expect_error(format_chr("xxx", italics = "xxx"),
                                "Argument `italics` must be TRUE or FALSE"))
  suppressMessages(expect_error(format_chr("xxx", type = "xxx"),
                                "Argument `type` must be 'md' or 'latex'"))
...
test_that("format_sub() formats properly", {
  suppressMessages(expect_error(format_sub(3),
                                "Input must be a character string"))
  suppressMessages(expect_error(format_sub("xxx", type = "xxx"),
                                "Argument `type` must be 'md' or 'latex'"))
