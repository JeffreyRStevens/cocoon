...
test_ttest3 <- suppressWarnings(wilcox.test(df$a, mu = 5, exact = TRUE))
test_ttest4 <- suppressWarnings(wilcox.test(df$a, df$b, exact = TRUE))
test_ttest5 <- suppressWarnings(wilcox.test(df$a, c(df$b, 120), exact = TRUE))
