...
The `format_stats()` function can also input objects returned by the `lm()`, `glm()`, `lme4::lmer()`, and `lme4::glmer()` functions. It can report overall model statistics (e.g., R-squared, AIC) for `lm()` and `glm()` and term-specific statistics (e.g., coefficients, p-values) for all models.
